...
Our experimental results demonstrate that ConsistAgent significantly improves cross-question consistency while maintaining competitive performance on traditional distribution matching metrics.
...
ConsistAgent maintains a consistency memory $M$ that tracks previous responses and applies constraints during response generation. For each new question $q$, ConsistAgent identifies relevant consistency constraints involving $q$, retrieves related previous responses from memory $M$, applies constraint-based filtering to response options, generates responses using weighted probabilities that favor consistent options, and updates memory $M$ with the new response.
