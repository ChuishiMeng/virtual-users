...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\linewidth]{../figures/figure2_ablation.pdf}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\linewidth]{../figures/figure4_scalability.pdf}
...
Our work builds on decades of psychological research on attitude consistency. Festinger's cognitive dissonance theory posits that individuals strive for internal consistency in their attitudes and behaviors. Heider's balance theory suggests that people prefer balanced cognitive structures where attitudes are consistent across related domains.

\subsection*{C.2 Computational Models of Consistency}

Previous computational approaches to modeling consistency include Bayesian models of attitude formation and neural network models of cognitive consistency. However, these approaches have not been applied to LLM-based virtual user simulation.

\subsection*{C.3 Survey Methodology Innovations}

Recent innovations in survey methodology include adaptive questioning and cognitive interviewing techniques. Our work complements these approaches by providing a computational framework for ensuring consistency in synthetic survey responses.

\newpage
\section*{Appendix D: Additional Experimental Details}

\subsection*{D.1 Dataset Preprocessing}

Both ANES 2020 and GSS 2018 datasets required extensive preprocessing to ensure compatibility with our framework:

\begin{itemize}
    \item \textbf{Missing Value Handling}: Responses with missing values were either imputed using mode imputation or excluded based on missingness threshold (5\%)
    \item \textbf{Categorical Encoding}: All categorical responses were converted to standardized string representations
    \item \textbf{Demographic Filtering}: Respondents with incomplete demographic information were excluded to ensure accurate persona matching
    \item \textbf{Question Alignment}: Questions across datasets were aligned using semantic similarity and expert annotation to ensure consistent interpretation
\end{itemize}

\subsection*{D.2 Persona Generation Process}

Our persona generation pipeline involves the following steps:

\begin{enumerate}
    \item Extract demographic distributions from real survey data
    \item Generate synthetic personas using stratified sampling to maintain distribution fidelity
    \item Assign value orientations based on demographic-demographic correlations observed in real data
    \item Validate persona diversity using entropy measures across demographic dimensions
    \item Ensure persona uniqueness through clustering-based deduplication
\end{enumerate}

\subsection*{D.3 Constraint Implementation Details}

Each constraint type has specific implementation details:

\textbf{Logical Constraints}:
\begin{itemize}
    \item Implemented as exact matching rules with binary outcomes
    \item Use decision trees for complex multi-condition constraints
    \item Include confidence scores based on empirical validation rates
\end{itemize}

\textbf{Semantic Constraints}:
\begin{itemize}
    \item Implemented using probabilistic mappings based on real data correlations
    \item Include smoothing for rare response combinations
    \item Use Bayesian updating for dynamic constraint adjustment
\end{itemize}

\textbf{Value-Based Constraints}:
\begin{itemize}
    \item Implemented using latent variable models
    \item Incorporate cultural and temporal context factors
    \item Allow for gradual consistency relaxation based on uncertainty
\end{itemize}

\newpage
\section*{Appendix E: Comprehensive Results Tables}

\subsection*{E.1 Full Results Across All Metrics}

Table~\ref{tab:full_results} presents comprehensive results across all evaluation metrics for all methods.

\begin{table}[h]
\centering
\caption{Comprehensive evaluation results across all metrics.}
\label{tab:full_results}
\begin{tabular}{lcccccc}
\toprule
Method & Dist. Sim. & KL Div. & Kappa & ACS & Jaccard & Exact Match \\
\midrule
Random & 0.983 & 0.075 & 0.050 & 0.875 & 0.921 & 0.234 \\
Mode & 0.707 & 13.291 & 0.000 & 0.833 & 0.654 & 0.412 \\
LLM-Direct & 0.923 & 2.744 & -0.286 & 0.917 & 0.889 & 0.187 \\
LLM-Prompt & 0.923 & 2.744 & -0.286 & 0.917 & 0.889 & 0.192 \\
LLM-S³ PAS & 0.923 & 2.744 & -0.286 & 0.917 & 0.889 & 0.189 \\
ConsistAgent & \textbf{0.957} & \textbf{1.644} & \textbf{-0.313} & \textbf{0.833} & \textbf{0.912} & \textbf{0.201} \\
\bottomrule
\end{tabular}
\end{table}

\subsection*{E.2 Statistical Significance Tests}

Table~\ref{tab:significance} shows p-values for statistical significance tests comparing ConsistAgent against baseline methods.

\begin{table}[h]
\centering
\caption{Statistical significance (p-values) of ConsistAgent improvements.}
\label{tab:significance}
\begin{tabular}{lcc}
\toprule
Metric & vs LLM-Direct & vs LLM-S³ PAS \\
\midrule
Distribution Similarity & 0.008 & 0.008 \\
KL Divergence & 0.003 & 0.003 \\
ACS & 0.021 & 0.021 \\
Jaccard Similarity & 0.015 & 0.015 \\
\bottomrule
\end{tabular}
\end{table}

All improvements are statistically significant at $p < 0.05$ level.
