#!/usr/bin/env python3
"""
Virtual Users Research - Main Experiment Runner
Version: v3 (Cross-Question Consistency Focus)

This script runs the complete virtual user experiment pipeline:
...
def run_baseline_methods(
    ...
    baselines = config.get(
        "baselines", ["random", "mode", "llm_direct", "llm_prompt", "llm_s3_pas"]
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run Virtual Users Research Experiment"
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    logger.info("Starting Virtual Users Research Experiment")

    try:
        # Initialize data loader
        data_config = config.get("data", {})
        # Load dataset using the convenience function
        data_config = config.get("data", {})
        dataset_name = data_config.get("name", "anes_2020")
        ...
        baseline_results = run_baseline_methods(
            config, dataset, persona_pool, logger
        )
