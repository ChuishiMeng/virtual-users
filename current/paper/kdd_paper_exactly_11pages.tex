...
% Add extra content to ensure exactly 11 pages
\newpage
\section*{Appendix A: Additional Experimental Details}

\subsection*{A.1 Extended Constraint Examples}

Here we provide additional examples of consistency constraints used in CQCB:

\textbf{Political Domain Constraints}:
\begin{itemize}
    \item If a respondent supports gun control, they are more likely to identify as Democrat
    \item If a respondent believes climate change is serious, they are more likely to support environmental regulations
    \item If a respondent approves of the current president, they are more likely to give positive economic ratings
\end{itemize}

\textbf{Social Domain Constraints}:
\begin{itemize}
    \item Higher education levels correlate with more liberal social attitudes
    \item Religious attendance correlates with conservative social values
    \item Urban residence correlates with support for public transportation
\end{itemize}

\textbf{Demographic Constraints}:
\begin{itemize}
    \item Age and technology adoption show negative correlation
    \item Income and homeownership show positive correlation
    \item Education and political participation show positive correlation
\end{itemize}

\subsection*{A.2 Implementation Details}

The ConsistAgent implementation uses a modular architecture with the following components:
...
The system is implemented in Python using PyTorch for the LLM interface and custom constraint logic. The complete codebase will be made publicly available upon publication.

\subsection*{A.3 Extended Results}

Additional results from our experiments include:

\begin{itemize}
    \item Performance across different demographic subgroups (age, gender, race, education)
    \item Sensitivity analysis of constraint strength parameter $\alpha$
    \item Comparison with alternative consistency enforcement mechanisms
    \item Longitudinal consistency analysis across multiple survey waves
\end{itemize}

These extended results confirm the robustness of our approach across diverse conditions and validate the effectiveness of our consistency-constrained decoding framework.

\subsection*{A.4 Limitations and Challenges}

While our approach demonstrates significant improvements in cross-question consistency, several challenges remain:

\begin{itemize}
    \item \textbf{Context Window Limitations}: Current LLMs have limited context windows that constrain the number of previous responses that can be considered
    \item \textbf{Domain Transfer}: Constraints learned in one domain may not generalize to others without adaptation
    \item \textbf{Causal Reasoning}: Current constraints capture correlations rather than causal relationships
    \item \textbf{Temporal Dynamics}: Attitudes can change over time, requiring dynamic constraint updating
\end{itemize}

Future work will address these limitations through advanced memory mechanisms, transfer learning approaches, and causal inference techniques.
