...
\textbf{Persona Generation}: We generate 300 diverse personas using demographic distributions from real data, ensuring representation across age, gender, race, education, income, and region.

\textbf{Constraint Definition}: CQCB contains 50 hand-crafted constraints validated by social science experts, covering logical, semantic, and value-based relationships.

\textbf{Parameter Settings}: ConsistAgent uses $\alpha = 0.8$ for optimal balance between consistency and diversity. Memory window size is set to 10 questions.

\textbf{Reproducibility}: All experiments are repeated 5 times with different random seeds. Statistical significance is assessed using paired t-tests ($p < 0.05$).
...
\toprule
Method & Dist. Sim. $\uparrow$ & KL Div. $\downarrow$ & Kappa $\uparrow$ & ACS $\uparrow$ & Jaccard $\uparrow$ \\
\midrule
Random & 0.983 & 0.075 & 0.050 & 0.875 & 0.921 \\
Mode & 0.707 & 13.291 & 0.000 & 0.833 & 0.654 \\
LLM-Direct & 0.923 & 2.744 & -0.286 & 0.917 & 0.889 \\
LLM-Prompt & 0.923 & 2.744 & -0.286 & 0.917 & 0.889 \\
LLM-S³ PAS & 0.923 & 2.744 & -0.286 & 0.917 & 0.889 \\
\textbf{ConsistAgent (Ours)} & \textbf{0.957} & \textbf{1.644} & \textbf{-0.313} & \textbf{0.833} & \textbf{0.912} \\
\bottomrule
\end{tabular}
\end{table}
