...
Consistency has been studied in various LLM contexts. Generative Agents \citep{park2023generative} introduce memory streams and reflection mechanisms to maintain consistent agent behavior over time. In dialogue systems, \citet{abdulhai2025measuring} propose conversation-level consistency metrics for evaluating LLM coherence.
...
Several benchmarks have been proposed for evaluating LLM agents. AlignSurvey \citep{liu2025alignsurvey} provides a comprehensive benchmark for human preferences alignment in social surveys. HumanStudy-Bench \citep{zhang2026humanstudy} focuses on AI agent design for participant simulation.
...
Traditional survey methodology has long recognized the importance of response consistency. Cronbach's alpha \citep{cronbach1951coefficient} measures internal consistency of survey items, while test-retest reliability \citep{nunnally1978psychometric} assesses consistency over time. However, these measures are designed for human respondents and cannot be directly applied to LLM-generated responses.
